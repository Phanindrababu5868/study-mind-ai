...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
  <>
  <div className={`fixed  inset-0 bg-black/30 z-40 md:hidden transition-opacity duration-200 
  ...
  <aside className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 md:relative md:w-64 md:shrink-0 md:flex md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out
    ${isSidebarOpen?'translate-x-0':'-translate-x-full'}`}>
      ...
      <div className="flex items-center justify-center h-16 px-5  border-slate-200/60">
      ...
      <nav className="flex-1  px-3 py-6 space-y-1.5">
        {navLinks.map((link)=>(
          ...
          ${isActive ? 'bg-linear-to-br from-emerald-500 to-teal-500 shadow-lg shadow-emerald-500/25 text-white':
                ...
                <>
                  <link.icon
                    ...
                    className={`transition-transform duration-200  ${
                      isActive ? 'text-white' : 'group-hover:scale-110 text-slate-900'
                    }`}
                  />
                  {link.text}
                </>
